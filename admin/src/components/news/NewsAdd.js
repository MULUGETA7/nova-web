...
const NewsAdd = () => {
  ...
  const handleImageChange = (e) => {
    ...
    if (file) {
      if (file.size > 5242880) {
        setError('Image dimension overflow (Max 5MB)');
        return;
      }
      if (!file.type.startsWith('image/')) {
        setError('Invalid asset format. Please use JPEG or PNG.');
        return;
      }
      setImage(file);
  ...
  const handleSubmit = async (e) => {
    ...
    if (!image) {
      setError('Visual asset (Image) is required for transmission');
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      setSuccess('Intelligence report indexed successfully');
      ...
      >
        ...
        <div className="flex items-center justify-between mb-12">
          <div>
            <h1 className="text-4xl font-black text-white tracking-tight italic">
              INITIATE <span className="nova-gradient-text uppercase">Transmission</span>
            </h1>
            <p className="text-gray-500 mt-1 font-medium uppercase tracking-[0.2em] text-[10px]">Create New Intelligence Report</p>
          </div>
        ...
        <div className="glass-card rounded-[3rem] p-10 md:p-16 relative overflow-hidden border-white/5">
          <div className="absolute top-0 right-0 p-12 opacity-[0.03] pointer-events-none">
            <NewspaperIcon className="w-64 h-64 text-white" />
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-10 relative z-10">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
              ...
              <div className="space-y-8">
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest ml-2">Intelligence Directive (Title)</label>
                  <input
                    type="text"
                    required
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest ml-2">Stream Classification</label>
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest ml-2">Temporal Marker (Publication Date)</label>
                  <div className="relative">
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest ml-2">Visual intelligence Asset</label>
                <div className="relative h-[calc(100%-24px)] min-h-[300px] group">
                  ...
                  />
                  <div className="h-full w-full bg-white/[0.03] border border-white/5 border-dashed group-hover:border-cyan-500/30 rounded-[2.5rem] flex flex-col items-center justify-center transition-all overflow-hidden">
                    {imagePreview ? (
                      ...
                      <>
                        <CloudArrowUpIcon className="w-12 h-12 text-gray-700 group-hover:text-cyan-400 transition-colors mb-4" />
                        <span className="text-[10px] font-black text-gray-600 uppercase tracking-[0.2em] group-hover:text-gray-400">Initialize Visual Stream</span>
                      </>
                    )}
                  </div>
                </div>
            ...
            <div className="space-y-4 pt-10 border-t border-white/5">
              <label className="flex items-center gap-3 text-[10px] font-black text-gray-500 uppercase tracking-widest ml-2">
                ...
                Intelligence Data Stream (Content)
              </label>
              <div className="nova-quill rounded-[2rem] overflow-hidden bg-white/[0.02] border border-white/5">
                <ReactQuill
                  ...
                  className="h-80"
                  readOnly={loading}
                  placeholder="Enter detailed report data..."
                />
              </div>
            </div>
            ...
            >
              ...
              {loading ? 'Transmitting Intelligence...' : 'Authorize and Publish Article'}
            </button>
          </form>
        </div>
      </motion.div>
