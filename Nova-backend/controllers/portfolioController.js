...
const createPortfolio = async (req, res) => {
  try {
    ...
    const portfolio = new Portfolio({ images: imageUrls });
    await portfolio.save();

    res.status(201).json({ message: "Portfolio image uploaded successfully", portfolio });
  } catch (error) {
...
const updatePortfolio = async (req, res) => {
  try {
    ...
    const updatedPortfolio = await Portfolio.findByIdAndUpdate(
      req.params.id,
      {
        images: imageUrls.length > 0 ? imageUrls : undefined, // Keep existing images if none are uploaded
      },
